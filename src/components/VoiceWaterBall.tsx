...
export default function VoiceWaterBall({ 
  ...
  const drawWaterBall = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const gradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, baseRadius * (1 + intensity))
    if (isSpeaking) {
    ...
    if (isSpeaking && intensity > 0.1) {
      ...
      ctx.beginPath()
      ctx.arc(centerX, centerY, baseRadius * 0.3 * (1 + intensity), 0, Math.PI * 2)
      ctx.fill()
    }
    ...
    if (isActive || isSpeaking) {
      ...
      for (let r = 0; r < 3; r++) {
        const rippleRadius = baseRadius * (1.5 + r * 0.3) * (1 + Math.sin(rippleTime - r) * 0.1)
        const rippleAlpha = Math.max(0, 0.3 - r * 0.1) * (isSpeaking ? intensity : 0.5)
    ...
    ctx.restore()
  }, [frequencyData, isSpeaking, isActive])
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      drawWaterBall(ctx, canvas.width, canvas.height)
  ...
  }, [isActive, analyser, drawWaterBall])
